...
RSpec.describe Myott::MyottController, type: :controller do
  describe '#current_user' do
    ...
    it 'verifies the ID token and returns the current user' do
      allow(CognitoTokenVerifier).to receive(:verify_id_token).with(decrypted_token).and_return('user_data')
