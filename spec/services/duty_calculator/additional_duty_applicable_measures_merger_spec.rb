RSpec.describe DutyCalculator::AdditionalDutyApplicableMeasuresMerger, :user_session do
  ...
  let(:uk_filtered_commodity) { DutyCalculator::Api::Commodity.build('uk', '0103921100') }
  let(:xi_filtered_commodity) { DutyCalculator::Api::Commodity.build('xi', '0103921100') }
  ...
  describe '#call' do
    ...
    context 'when on an xi route' do
      let(:user_session) { build(:duty_calculator_user_session, commodity_source: 'xi', commodity_code: '0103921100') }
    ...
    context 'when on a uk route' do
      let(:user_session) { build(:duty_calculator_user_session, commodity_source: 'uk', commodity_code: '0103921100') }
