      ...
      VCR.use_cassette('geographical_areas#countries') do
        VCR.use_cassette('sections#index') do
          VCR.use_cassette('search#gold') do
            ...
            page.find('.hott-search-banner__toggle').click

            page.find('.hott-search-banner .autocomplete__input').click

            page.find('.hott-search-banner .autocomplete__input').set('gold')
            sleep 1

            expect(page.find('.hott-search-banner .autocomplete__option--focused').text).to eq('gold')

            using_wait_time 1 do
              expect(page.find_all('.hott-search-banner .autocomplete__option').length).to be > 1
            end

            expect(page.find('.hott-search-banner .autocomplete__option--focused').text).to eq('gold')
            expect(page).to have_content('gold - gold coin')

            page.find('.hott-search-banner .autocomplete__option--focused').click
      ...
      VCR.use_cassette('geographical_areas#countries') do
        VCR.use_cassette('sections#index') do
          VCR.use_cassette('search#gibberish') do
            ...
            page.find('.hott-search-banner .autocomplete__input').set('dsauidoasuiodsa')

            sleep 1

            expect(page.find_all('.hott-search-banner .autocomplete__option').length).to eq(1)
            expect(page.find('.hott-search-banner .autocomplete__option--focused').text).to eq('dsauidoasuiodsa')
            sleep 1
