...
RSpec.describe MeasuresHelper, type: :helper do
  ...
  describe '#export_measures_heading' do
    before { allow(helper).to receive(:measures_heading).with(anchor: 'export').and_return('Exporting from the UK') }
    ...
    it 'includes the all-countries context and date' do
      ...
      expect(helper.export_measures_heading(search)).to eq('Exporting from the UK to All countries on 22 July 2026')
    end

    it 'includes the selected country' do
      search = instance_double(Search, geographical_area: instance_double(GeographicalArea, description: 'France'), date:)

      expect(helper.export_measures_heading(search)).to eq('Exporting from the UK to France on 22 July 2026')
    end
  end
