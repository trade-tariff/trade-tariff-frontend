...
Current eligibility is checked for **every new submission**, including follow-up answers. Disabling guided search prevents new work but lets a previously accepted step finish with its valid grant. The Flagsmith kill switch is enqueue-only: issued grants can poll and hand off for up to one hour. Polling therefore does not repeat remote Flagsmith identity calls. Ordinary page/final-navigation setup can still evaluate flags; these are not zero-dependency requests.
...
- Malformed completed envelopes, resource shapes or consumed metadata produce recovery, not a cached success or misleading no-results page. Invalid expansion reporting data is removed and does not fail the result. Backend request identifiers retain the existing 64-character restriction; pending questions need selectable, nonblank options. Exclusion copy remains optional: without both message fields, preserve the existing no-results route.
- Only explicit pre-enqueue `validation_failed: true` JSON responses use the ordinary form post to preserve existing GOV.UK field/date errors. A non-JSON 422 (including Rails CSRF `InvalidAuthenticityToken`) reloads the page for a fresh token. Do not skip CSRF on enqueue. Generic JSON 422s and execution/network errors never silently start synchronous search. Malformed completed payloads return `422` with `error_code: invalid_response` so the poller fails immediately; transient backend `503` still uses bounded backoff.
